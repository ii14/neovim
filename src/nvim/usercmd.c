...
int uc_add_command(char *name, size_t name_len, const char *rep, uint32_t argt, long def, int flags,
                   int compl_, char *compl_arg, LuaRef compl_luaref, LuaRef preview_luaref,
...
void ex_command(exarg_T *eap)
{
  ...
  while (*p == '-') {
    ...
    if (uc_scan_attr(p, (size_t)(end - p), &argt, &def, &flags, &compl_, (char_u **)&compl_arg,
                     &addr_type_arg) == FAIL) {
      return;
    }
    p = skipwhite(end);
  ...
  } else if (compl_ > 0 && (argt & EX_EXTRA) == 0) {
    emsg(_(e_complete_used_without_allowing_arguments));
  } else {
    uc_add_command(name, name_len, p, argt, def, flags, compl_, compl_arg, LUA_NOREF, LUA_NOREF,
